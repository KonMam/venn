...
function tdiffKey(cell) {
  var v = cell.getAttribute("data-v");
  return (v !== null ? v : cell.textContent).trim();
}
function tdiffNum(s) { return Number(String(s).replace(/,/g, "")); }
function tdiffCmp(x, y, numeric) {
  var nx = tdiffNum(x), ny = tdiffNum(y);
  ...
  table.querySelectorAll("th").forEach(function (th, i) {
    ...
    th.addEventListener("click", function () {
      ...
      rows.sort(function (a, b) {
        var c = tdiffCmp(tdiffKey(a.cells[i]), tdiffKey(b.cells[i]), numeric);
        return desc ? -c : c;
      });
...
func HTML(w io.Writer, res *diff.Result, left, right string) {
	...
	fmt.Fprintf(w, "<title>tdiff: %s vs %s</title>\n<style>%s</style>\n</head>\n<body>\n<main>\n",
		esc(left), esc(right), htmlStyle)

	fmt.Fprintf(w, "<h1>tdiff report</h1>\n")
...
func htmlNotes(w io.Writer, res *diff.Result) {
	...
	if res.Filter != "" {
		note := "<b>Filtered to</b> <code>" + esc(res.Filter) +
			"</code> on both sides, so every count is of the matching rows"
...
func htmlExamples(w io.Writer, res *diff.Result) {
	...
	fmt.Fprintf(w, "<footer>Generated by tdiff. Self-contained: no external requests.</footer>\n")
}
